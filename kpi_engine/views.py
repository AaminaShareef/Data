...
# Works whether cleaned_file is a FileField, string, or relative media path
# ─────────────────────────────────────────────────────────────────────────────
def _resolve_cleaned_path(cleaning_report):
    ...
    # Case 1: Django FileField with .path attribute
    try:
        p = cf.path
    ...
    # Case 2: Plain string — may be absolute already
    cf_str = str(cf).strip()
...
def interactive_dashboard(request, result_id):
    ...
    # ── Load cleaned file (robust path resolution) ────────────────────────────
    df = pd.DataFrame()
    try:
    ...
    # ── Column metadata (skip outlier flags and date-extracted cols) ──────────
    columns_meta = {}
    ...
    # ── Sanitise: keep only mapped cols, replace NaN/Inf with None ───────────
...
def ai_insight(request, result_id):
    ...
    # Step 1 — hardcoded instant insight
...
def generate_report(request, result_id):
    """
    Receives report payload from the interactive dashboard JS,
    builds a styled PDF using matplotlib + reportlab, and streams it back
    as a file download.
    ...
    include_kpis     = body.get('include_kpis', True)
    include_charts   = body.get('include_charts', True)
    include_narrative= body.get('include_narrative', True)
    include_insights = body.get('include_insights', True)
    include_table    = body.get('include_table', True)

    result_json  = result.result_json
    dataset_name = result.cleaning_report.dataset.name \
                   if hasattr(result.cleaning_report.dataset, 'name') \
                   else f'Dataset #{result.cleaning_report.dataset_id}'

    # Build payload for report_generator
    payload = {
        ...
        'kpis':           result_json.get('kpis', [])       if include_kpis      else [],
        'insights':       result_json.get('insights', [])   if include_insights  else [],
        'narrative':      narrative                          if include_narrative else '',
        'filtered_data':  filtered_data                     if include_table     else [],
    ...
    try:
        ...
        return JsonResponse(
            {'error': 'Report generator not installed. Run: pip install reportlab matplotlib'},
            status=500
        )
    except Exception as e:
    ...
    filename = f"auralis_report_{datetime.now().strftime('%Y-%m-%d')}.pdf"
...
# ─────────────────────────────────────────────────────────────────────────────
# VIEW 5 — Generate PDF Report
# URL: /analysis/result/<result_id>/generate-report/
# ─────────────────────────────────────────────────────────────────────────────
@require_POST
def generate_report(request, result_id):
    """
    ...
    # Use filtered data from request if provided, else load full cleaned file
    filtered_data = body.get('filtered_data', [])
    ...
    # Rebuild columns_meta from data if not sent
